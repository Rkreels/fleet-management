...
const initialEntries = [
  { id: 1, vehicle: 'TN 09 AB 1234', driver: 'Rajan Kumar', date: '2026-01-15', km: 48320, fuel: 85, cost: 8415, kmpl: 4.9, vendor: 'HP Petrol Pump, Chennai' },
  { id: 2, vehicle: 'TN 09 CD 5678', driver: 'Suresh Babu', date: '2026-01-14', km: 62150, fuel: 92, cost: 9108, kmpl: 4.5, vendor: 'BPCL, Coimbatore' },
  { id: 3, vehicle: 'TN 09 EF 9012', driver: 'Murugan S', date: '2026-01-14', km: 31200, fuel: 78, cost: 7722, kmpl: 5.1, vendor: 'IOC, Madurai' },
  { id: 4, vehicle: 'TN 09 IJ 7890', driver: 'Karthik M', date: '2026-01-13', km: 22400, fuel: 65, cost: 6435, kmpl: 5.3, vendor: 'HP, Salem' },
  { id: 5, vehicle: 'TN 09 AB 1234', driver: 'Rajan Kumar', date: '2026-01-10', km: 47800, fuel: 88, cost: 8712, kmpl: 4.8, vendor: 'BPCL, Chennai' },
];
...
const Fuel: React.FC = () => {
  ...
  const [form, setForm] = useState({ vehicle: '', driver: '', km: '', fuel: '', cost: '', vendor: '' });
  ...
  const handleAdd = () => {
    if (!form.vehicle || !form.km || !form.fuel || !form.cost) { toast.error('Fill all required fields'); return; }
    const newEntry = {
      id: entries.length + 1,
      ...
      kmpl: parseFloat(form.km) > 0 && parseFloat(form.fuel) > 0 ? parseFloat((parseFloat(form.km) / parseFloat(form.fuel)).toFixed(1)) : 0,
      vendor: form.vendor || '-',
    };
  ...
  const handleDownload = () => {
    const csv = ['Vehicle,Driver,Date,KM,Fuel(L),Cost(₹),KMPL,Vendor', ...entries.map(e => `${e.vehicle},${e.driver},${e.date},${e.km},${e.fuel},${e.cost},${e.kmpl},${e.vendor}`)].join('\n');
      ...
      <div className="space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-slate-500 text-sm">{entries.length} fuel entries</p>
          </div>
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Monthly Fuel Cost', value: '₹71,000', icon: FuelIcon, color: 'bg-orange-500' },
            { label: 'Avg KMPL', value: '4.9', icon: TrendingUp, color: 'bg-green-600' },
            { label: 'Total Fuel (L)', value: '7,245', icon: FuelIcon, color: 'bg-blue-600' },
            { label: 'Entries This Month', value: '48', icon: FuelIcon, color: 'bg-purple-600' },
          ].map(k => (
        ...
        <div className="relative max-w-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search entries..." className="w-full pl-9 pr-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#f97316]/40" />
        </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((entry, i) => (
                  <motion.tr key={entry.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="hover:bg-slate-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-sm">
                      <span className={`font-bold ${entry.kmpl >= 5 ? 'text-green-600' : entry.kmpl >= 4.5 ? 'text-orange-600' : 'text-red-600'}`}>{entry.kmpl}</span>
                    </td>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6">
              ...
              <div className="flex gap-3 mt-5">
                ...
                <button onClick={handleAdd} className="flex-1 py-2.5 bg-[#f97316] text-white rounded-lg hover:bg-[#ea6c0a] transition-all text-sm font-medium">Add Entry</button>
              </div>
            </div>
          </div>
        )}
