...
const initialTrips = [
  { id: 1, vehicle: 'TN 09 AB 1234', driver: 'Rajan Kumar', from: 'Chennai', to: 'Bangalore', date: '2026-01-15', distance: 350, freight: 28000, fuelCost: 9800, tollCost: 1200, otherCost: 800, status: 'active' as const },
  { id: 2, vehicle: 'TN 09 CD 5678', driver: 'Suresh Babu', from: 'Coimbatore', to: 'Hyderabad', date: '2026-01-14', distance: 620, freight: 45000, fuelCost: 17360, tollCost: 2400, otherCost: 1200, status: 'active' as const },
  { id: 3, vehicle: 'TN 09 EF 9012', driver: 'Murugan S', from: 'Madurai', to: 'Chennai', date: '2026-01-13', distance: 460, freight: 32000, fuelCost: 12880, tollCost: 1800, otherCost: 600, status: 'inactive' as const },
  { id: 4, vehicle: 'TN 09 IJ 7890', driver: 'Karthik M', from: 'Salem', to: 'Pune', date: '2026-01-12', distance: 1100, freight: 72000, fuelCost: 30800, tollCost: 4200, otherCost: 2000, status: 'inactive' as const },
  { id: 5, vehicle: 'TN 09 KL 2345', driver: 'Selvam P', from: 'Chennai', to: 'Delhi', date: '2026-01-10', distance: 2180, freight: 125000, fuelCost: 61040, tollCost: 8400, otherCost: 4500, status: 'inactive' as const },
];
...
const Trips: React.FC = () => {
  ...
  const [form, setForm] = useState({ vehicle: '', driver: '', from: '', to: '', distance: '', freight: '', fuelCost: '', tollCost: '', otherCost: '' });
  ...
  const handleAdd = () => {
    if (!form.vehicle || !form.from || !form.to) { toast.error('Fill required fields'); return; }
    ...
    setForm({ vehicle: '', driver: '', from: '', to: '', distance: '', freight: '', fuelCost: '', tollCost: '', otherCost: '' });
    setAddOpen(false);
    toast.success('Trip added!');
  };

  const handleDownload = () => {
    const csv = ['Vehicle,Driver,From,To,Date,Distance,Freight,Fuel Cost,Toll,Other,Net P/L', ...trips.map(t => {
    ...
    <Layout>
      <div className="space-y-6">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          {[
            { label: 'Total Freight', value: `₹${totalFreight.toLocaleString()}`, color: 'bg-green-600', icon: TrendingUp },
            { label: 'Total Expenses', value: `₹${totalExpense.toLocaleString()}`, color: 'bg-red-500', icon: TrendingDown },
            { label: 'Net P&L', value: `₹${netPL.toLocaleString()}`, color: netPL >= 0 ? 'bg-blue-600' : 'bg-red-600', icon: TrendingUp },
        ...
        <div className="relative max-w-sm">
          ...
          <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search trips..." className="w-full pl-9 pr-4 py-2.5 border border-slate-300 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-[#f97316]/40" />
        </div>
            ...
            <table className="w-full">
              ...
              <tbody className="divide-y divide-slate-100">
                {filtered.map((trip, i) => {
                  ...
                  return (
                    <motion.tr key={trip.id} initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: i * 0.03 }} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-slate-600 text-sm whitespace-nowrap">{trip.from} → {trip.to}</td>
                      ...
                      <td className="px-4 py-3 font-bold text-sm">
                        <span className={pl >= 0 ? 'text-green-700' : 'text-red-600'}>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md p-6 max-h-[90vh] overflow-y-auto">
              ...
              <div className="flex gap-3 mt-5">
                ...
                <button onClick={handleAdd} className="flex-1 py-2.5 bg-[#f97316] text-white rounded-lg hover:bg-[#ea6c0a] transition-all text-sm font-medium">Add Trip</button>
              </div>
            </div>
          </div>
        )}
